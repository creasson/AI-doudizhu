...
class Doudizhu(object):
    ...
    def list_greater_cards(cards_target, cards_candidate):
        ...
        for card_type, weight in target_type:
            ...
            # 修正"连三4连"(trio_chain_4)的跟牌可能出现"连三带一3连"(trio_solo_chain_3)的BUG。
            if card_type == 'trio_solo_chain_3' and 'trio_chain_4' in [stype[0] for stype in target_type]:
                cards_gt.pop(card_type)
    ...
    def list_valid_cards(cards_candidate):
        ...
        for card_type in Doudizhu.TYPE_CARDS:
            weight_vt = Doudizhu.TYPE_CARDS[card_type]
            if card_type not in valid_cards:
                valid_cards[card_type] = []
            for w in weight_vt:
                for w_cards in Doudizhu.TYPE_CARDS[card_type][w]:
                    w_cardmap = str2cardmap(w_cards)
                    if Doudizhu.cards_contain(candidate_cardmap, w_cardmap):
                        valid_cards[card_type].append(w_cards)
            if not valid_cards[card_type]:
                valid_cards.pop(card_type)
        return valid_cards

    @staticmethod
    def laizi_cards_contain(laizi, candidate_cardmap, cardmap):
        """判断所选牌是否属于候选牌"""

        # 候选牌不含癞子，按正常逻辑判断
        if laizi not in candidate_cardmap:
            return Doudizhu.cards_contain(candidate_cardmap, cardmap)
        ...
        for k, v in iter(cardmap.items()):
            if k not in candidate_cardmap:
                laizi_count += v
            elif candidate_cardmap[k] < v and k != laizi:
                laizi_count += (v-candidate_cardmap[k])
            elif k == laizi:
                laizi_count += v
            if laizi_count > candidate_cardmap[laizi]:
                return False

        # 当只有癞子时，癞子不能替换为其他牌
        if laizi_count == sum(cardmap.values()):
            if laizi not in cardmap or laizi_count != cardmap[laizi]:
                return False

        return True

    @staticmethod
    def list_valid_laizi_cards(laizi, cards_candidate):
        """
            癞子斗地主: 从候选牌cards_candidate中找出所有有效的癞子出牌
        """
        valid_cards = {}
        candidate_cardmap = str2cardmap(cards_candidate)
        for card_type in Doudizhu.TYPE_CARDS:
            ...
            for w in weight_vt:
                for w_cards in Doudizhu.TYPE_CARDS[card_type][w]:
                    w_cardmap = str2cardmap(w_cards)
                    if Doudizhu.laizi_cards_contain(laizi, candidate_cardmap, w_cardmap):
                        valid_cards[card_type].append(w_cards)
            if not valid_cards[card_type]:
                valid_cards.pop(card_type)
        return valid_cards
    ...
    @staticmethod
    def list_greater_laizi_cards(laizi, cards_target, cards_candidate):
        """ 癞子斗地主: 对于目标牌组合cards_target
        从候选牌cards_candidate中找出所有可以压制它的牌型
